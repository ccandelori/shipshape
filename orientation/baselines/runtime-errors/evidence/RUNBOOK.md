...
Captured 2026-05-19 via `node orientation/baselines/runtime-errors/scenarios.mjs` against the live localhost stack (API :3000, web :5174). Driver script uses Playwright's `playwright` package (not the MCP — the MCP requires Chrome stable and the dev box has only the chromium-1200 cache). Every scenario writes a markdown evidence file plus a screenshot. Repro: `node orientation/baselines/runtime-errors/scenarios.mjs only=3,4,6,7,9,10` (sub-second feedback per scenario; ~80 s total).
...
## Scenarios already captured in the prior session

| # | Scenario | Evidence |
|---|---|---|
| 1 | WS session expiry mid-edit | Code + grep confirmed: WS validated only at upgrade in `api/src/collaboration/index.ts:347–393`; no per-message session re-check, no `setInterval` over `conns`, no `ws.ping()` heartbeat. |
| 2 | Two browser tabs editing same field (title LWW) | Code-confirmed: `Editor.tsx:187,810-814` — title is plain `useState` + debounced REST PATCH; not Yjs-bound. |
...
| 8† | Network 500 from API → HTML | `api-500-html-response.txt` (counter-example showing the proper JSON 500 path) + `csrf-html-response.txt` (the HTML 4xx case). |
...
node orientation/baselines/runtime-errors/scenarios.mjs              # all 6
node orientation/baselines/runtime-errors/scenarios.mjs only=3,9     # subset
```
