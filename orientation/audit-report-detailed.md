...
**Important percentile caveat:** `autocannon` JSON emits `p50`, `p90`, `p97_5`, and `p99`, but not native `p95`. The PRD's improvement target language ("20% P95 reduction") therefore cannot be reported against an actual P95 from these tools — `p97_5` is reported in tables below as the measured value, and serves as a strict upper bound on P95 (because `p97_5 ≥ p95` by definition for any latency distribution). Any improvement target stated in terms of `p97_5` is the literal Phase 1 metric; Phase 2 should plan to either (a) use a tool with native P95 emission (k6, wrk2), or (b) compute P95 from raw latency histograms, before stating an improvement claim *against P95 specifically*.
