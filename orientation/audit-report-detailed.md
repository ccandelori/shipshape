# ShipShape Audit Report — Detailed Reference

> **Executive summary:** `orientation/audit-report.md`. This is the full-prose / detailed reference version. The exec summary was restructured on 2026-05-20 for executive read-time; this file preserves the pre-restructure prose with the same fixes folded in.
...
**Audit baseline complete:** 2026-05-19 19:30 CT
**Live-evidence gaps closed via re-audit:** 2026-05-20
**Phase 1 deadline:** Tuesday, 2026-05-20 11:59 PM CT — **MET on 2026-05-20**
...
Phase 1 of the ShipShape audit on `US-Department-of-the-Treasury/ship` is **measured and met against the PDF PRD as of 2026-05-20**. All 7 categories carry baseline evidence. Live runs covered: type-safety (`pnpm type-check` exit 0 across `api`/`web`/`shared`; `e2e/` known scope gap, not in workspace), bundle size (`pnpm build:web` → 4.5 MB total, **2,074 KB / 588 KB gzipped main chunk**; treemap regenerable via `orientation/baselines/bundle/regenerate.sh`), API response time (15 autocannon JSONs at c=10/25/50; `/api/issues` slowest at **58 ms p97.5** c=50 — autocannon doesn't emit P95; p97.5 is a strict upper bound), database query efficiency (**exact per-flow query counts via `pg_stat_statements`** — 26 / 7 / 5 / 21 / 5 across the 5 canonical user flows — plus 5 EXPLAIN ANALYZE plans on the slowest query per flow; Memoize 0/139 hits on projects subquery, 92 of 104 issues post-filtered on dashboard query, Seq Scan on `document_associations` confirmed for team-grid), test coverage (api 40.34% line / 33.44% branch, 451/451 pass; **web 28.28% line / 18.99% branch, 151/151 pass** after the four audit-period test edits were reverted on master; 3 E2E runs at 864–866/869 pass with 3–5 reported flakes depending on run), runtime errors (12 of 12 scenarios captured live: 8 against unmodified master via Playwright/psql/curl, 1 via direct DB session-delete to force the 15-min HTTP idle expiry (WS-session-expiry — **bug confirmed live: REST 401 but WS kept persisting**), 1 via documented defect-injection-then-revert protocol (`yjsToJson` NULL — **bug confirmed live: `content IS NULL`, `yjs_state` survives 96 bytes**). Other live runs: disconnect/reconnect convergence, slow-3G blank screen, 10 KB title → 400 silent autosave fail, HTML/script injection no XSS, malformed Yjs → empty fallback, visibility race → WS close + post-flip drop, two-tab title race → last-write-wins confirmed. **Plus the PDF-required normal-usage console pass**: 11 walked routes, 1 expected `/api/auth/me` 401 at `/login`, 0 warnings, 0 page errors), and accessibility (Lighthouse 13.3.0 on 10 routes + **@axe-core/playwright deep scan on 8 authenticated routes** + **keyboard-only walkthroughs of 3 representative flows** + **real VoiceOver speech-log transcript** captured via guidepup + Playwright on `/my-week` and the document editor).
...
| 3 | Runtime | WebSocket session validated only at HTTP upgrade (`collaboration/index.ts:347–393`). A user whose session expires keeps editing until the browser closes. **Live-confirmed 2026-05-20**: destroyed-session user persisted typed phrase to `documents.content` despite REST 401. Evidence: `evidence/ws-session-expiry.md`. | Critical (security exposure) |
| 4 | Runtime | `yjsToJson()` has no try/catch around `JSON.stringify`; `undefined` returns silently null the `content` column while `yjs_state` survives. **Live-confirmed 2026-05-20** via documented defect-injection protocol (revertable): `documents.content IS NULL`, `yjs_state` survives at 96 bytes. Evidence at `evidence/yjs-to-json-null.md`. | Critical (silent data loss) |
...
| 9 | Tests | Web suite found broken on audited commit `076a18371` (13/146 failing across 4 of 16 files). Three distinct diagnoses, each verified against `git log`: 10 test-stale-post-intentional-prod-change, 3 born-broken authoring errors in `DetailsExtension.test.ts`, 1 born-broken under jsdom in `drag-handle.test.ts`. Repair is a Phase 2 deliverable; baseline preserves the as-found state. Meta-finding: web tests live but root `pnpm test` only runs `@ship/api`, so silently-rotting and dead-on-arrival tests went undetected. **API coverage: 40.34% line / 33.44% branch, bimodal** (hot routes 60–100%, cold routes <10%). | High |
...
**Coverage:** all 12 scenarios in the Cat 6 verification table below now have direct live evidence (Playwright/curl/psql), with #2's "live" caveated as defect-injection-protocol live rather than unmodified-stack live. The earlier "10/10 live or live-equivalent" framing is replaced by this honest accounting.
...
**Real VoiceOver transcript:** the PDF explicitly asks for screen-reader testing with VoiceOver/NVDA or similar on **the dashboard and a document edit page**. This audit satisfies that requirement via `orientation/baselines/accessibility/voiceover-walk.mjs` — guidepup drives a headed Chromium under Playwright, starts VoiceOver, walks **`/dashboard` (the PRD-literal path)**, `/my-week` (the actual default landing), and a wiki document editor with `VO+→`, and records the official macOS speech log via `voiceOver.spokenPhraseLog()`. The curated canonical summary lives at `orientation/baselines/accessibility/voiceover-results-2026-05-20.md`. The transcript surfaces six Phase 2 candidates (e.g. per-day buttons announce as "Mon5/18" with no whitespace; the editor body has no aria-labeled landmark). axe and keyboard testing remain in the evidence set as complementary signals, not proxies.
...
Candidate improvements ranked by category-target impact + delivery confidence + effort (inverted: easy is higher) + reproducibility of the before/after benchmark. The ranking below is ordinal; the multiplied scores are an artifact of the original ranking pass, not a unit-meaningful metric. Tiebreak: at least one Phase 2 deliverable in *every* category is mandatory.
...
1. **Type Safety:** TS-1 (extend root tsconfig + typed pg-mock helper) — Baseline: **747** violations (~498 production / ~249 test) → target: **560** (–25%, projected –150 from helper, –safety from tsconfig)
...
3. **API:** API-1 (session-activity write throttle) — Baseline: `/api/issues` **P95 = 46.6 ms** at c=50 (k6); `/api/documents?type=wiki` **P95 = 53.0 ms** at c=50 (k6); autocannon p97.5 for the remaining 3 endpoints in `baselines/api-response-time/`. Target: **–20% P95 on ≥2 endpoints** (measure with same k6 driver post-throttle).
4. **DB:** DB-1 (expression indexes migration 038) — Baseline: dashboard active-issues query plan in `baselines/explain-flow-3.txt` (and `db-efficiency/explain-dashboard-issues.txt`) — Bitmap Heap Scan via `idx_documents_document_type` over all 104 issue docs; **Rows Removed by Filter: 92** (88% filtered post-index-scan via JSONB `state`/`assignee_id`); Execution Time 0.149 ms warm-cache. Target: **≥50% improvement on the "my active issues" query** (re-EXPLAIN at the same shape post-index).
...
- [x] **Category 3 — API Response Time** — **MEASURED 2026-05-19 (with caveats).** 850,757 requests across 5 endpoints × c=10/25/50, all 2xx. **Headline**: `/api/issues` is the slowest endpoint (p97.5 = 11/37/58 ms at c=10/25/50); `/api/auth/me` is fastest (p97.5 = 6/12/22 ms). Latency scales roughly linearly with concurrency for all endpoints. **Caveats**: localhost + warm cache + 500 seeded documents / 104 issues / 20 users / 35 sprints + 10-core dev machine — the *absolute* ms numbers are a best-case floor, NOT representative of production; the *relative* rankings are robust. **Metric:** autocannon does not emit native P95; p97.5 is the measured value (and a strict upper bound on P95). **Rate-limit scaffold:** `X-Bench: 1` header + guarded `skip: isTestEnv && ...` in `api/src/app.ts` (added 2026-05-20) makes the benchmark reproducible from clean master with `E2E_TEST=1 pnpm dev:api` — see Cat 3 methodology for the full defense.
