  ...
  try {
    ...
    for (let i = 0; i < shipCoreIssues.length; i++) {
      ...
      if (assignedUserIndex < 0) {
        const assigneeName = 'assigneeName' in issue ? issue.assigneeName : `team index ${shipCoreTeam[i % shipCoreTeam.length]!}`;
        throw new Error(`Ship Core seed assignee not found: ${assigneeName}`);
      }
