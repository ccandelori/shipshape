...
describe('authMiddleware', () => {
  ...
  describe('session validation', () => {
    ...
    it('returns 401 when session does not exist in database', async () => {
      ...
      mockedPool().mockResolvedValueOnce({ rows: [], rowCount: 0 } as any);
      await authMiddleware(req, res, next);
      expect(res.status).toHaveBeenCalledWith(401);
      expect(res.json).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('error handling', () => {
    it('returns 500 on database error', async () => {
      ...
      mockedPool().mockRejectedValueOnce(new Error('DB connection failed'));
      await authMiddleware(req, res, next);
  ...
  describe('bearer token authentication', () => {
    ...
    it('returns 401 for revoked bearer token', async () => {
      ...
      mockedPool().mockResolvedValueOnce(pgResult([])); // No results means revoked/expired
