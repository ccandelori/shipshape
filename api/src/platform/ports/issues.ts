// Narrow IssuePort for public DIP. Reuses documents table (document_type='issue').
// Publish after txn success only. Minimal flat props for public DX.
...
export interface IssuePort {
  create(input: { title: string; workspaceId: string; authorId?: string | null; state?: string; priority?: string; assignee_id?: string | null }): Promise<{ id: string; title: string; state?: string; priority?: string; assignee_id?: string | null; created_at?: string }>;
  getById(id: string, workspaceId: string): Promise<{ id: string; title: string; state?: string; priority?: string; assignee_id?: string | null; created_at?: string; updated_at?: string } | null>;
  list(params: { workspaceId: string; cursor?: Cursor; limit?: number }): Promise<{ items: Array<{ id: string; title: string; state?: string; priority?: string; assignee_id?: string | null; created_at?: string }>; nextCursor?: Cursor }>;
  update(id: string, workspaceId: string, patch: { title?: string; state?: string; priority?: string; assignee_id?: string | null }): Promise<{ id: string; title: string; updated_at?: string } | null>;
  delete(id: string, workspaceId: string): Promise<boolean>;
}

function extractIssuePublic(row: any) {
...
export const issuePort: IssuePort = {
  async create(input) {
  ...
  async getById(id, workspaceId) {
    ...
    const row = res.rows[0];
    return row ? extractIssuePublic(row) : null;
  },
  ...
  async list(params) {
    ...
    const items = (hasMore ? rows.slice(0, limit) : rows).map(extractIssuePublic);
  ...
  async update(id, workspaceId, patch) {
    ...
    try {
      ...
      // read current props for merge
      const curRes = await client.query(
        `SELECT properties FROM documents WHERE id=$1 AND workspace_id=$2 AND document_type='issue' AND deleted_at IS NULL`,
        [id, workspaceId]
      );
      ...
      if (patch.state !== undefined) newProps.state = patch.state;
      if (patch.priority !== undefined) newProps.priority = patch.priority;
      if (patch.assignee_id !== undefined) newProps.assignee_id = patch.assignee_id;

      const sets: string[] = ['properties = $3', 'updated_at = now()'];
      const vals: any[] = [id, workspaceId, JSON.stringify(newProps)];
      if (patch.title !== undefined) {
        sets.unshift('title = $4');
        vals.push(patch.title);
      }

      const sql = `UPDATE documents SET ${sets.join(', ')} WHERE id = $1 AND workspace_id = $2 AND document_type = 'issue' AND deleted_at IS NULL RETURNING id, title, updated_at, properties`;
